/* ROS 2 COMMENTED OUT AT BOTTOM DONE
...
float ATRVJR::getDistance() {
    ...
    return (distance - first_distance) / static_cast<float>(ODO_DISTANCE_CONVERSION);
}

float ATRVJR::getBearing() {
    return (bearing - HOME_BEARING) / static_cast<float>(ODO_ANGLE_CONVERSION);
}

float ATRVJR::getTranslationalVelocity() const {
    return transVelocity / static_cast<float>(ODO_DISTANCE_CONVERSION);
}

float ATRVJR::getRotationalVelocity() const {
    return rotVelocity / static_cast<float>(ODO_ANGLE_CONVERSION);
}
...
void ATRVJR::getBodySonarPoints(sensor_msgs::msg::PointCloud* cloud) const {
    getSonarPoints(BODY_INDEX, cloud);
}
...
void ATRVJR::setMovement(float tvel, float rvel, float acceleration) {
...
void ATRVJR::getSonarReadings(const int ringi, float* adjusted_ranges) const {
    ...
    for (int x = SONAR_RING_BANK_BOUND[ringi]; x < SONAR_RING_BANK_BOUND[ringi + 1]; x++) {
        for (int y = 0; y < SONARS_PER_BANK[x]; y++) {
            ...
            if (range > SONAR_MAX_RANGE)
                range = SONAR_MAX_RANGE;
            float fRange = range / static_cast<float>(RANGE_CONVERSION);
            adjusted_ranges[i] = fRange;
            i++;
        }
    }
...
void ATRVJR::getSonarPoints(const int ringi, sensor_msgs::msg::PointCloud* cloud) const {
    int numSonar = SONARS_PER_RING[ringi];
    float* readings = new float[numSonar];
    getSonarReadings(ringi, readings);
    cloud->points.resize(numSonar);
    ...
    for (int i = 0; i < numSonar; i++) {
        if (readings[i] < SONAR_MAX_RANGE / static_cast<float>(RANGE_CONVERSION)) {
            angle = SONAR_RING_ANGLES[i];
            if (angle < 0) {
                angle += 2 * M_PI;
            }
            ...
            x /= 1000;
            y /= 1000;
            sonar_ring_distance[i] = std::sqrt((x * x) + (y * y));

            distance_from_center_of_rover = sonar_ring_distance[i] + readings[i];
            cloud->points[c].x = std::cos(angle) * distance_from_center_of_rover;
            cloud->points[c].y = std::sin(angle) * distance_from_center_of_rover;
            cloud->points[c].z = SONAR_RING_HEIGHT[ringi];
            c++;
        }
    }
...
void ATRVJR::processDioEvent(unsigned char address, unsigned short data) {
    if (address == HEADING_HOME_ADDRESS) {
        home_bearing = bearing;
        printf("ATRVJR Home %f \n", home_bearing / static_cast<float>(ODO_ANGLE_CONVERSION));
    } else {
        printf("ATRVJR DIO: address 0x%02x (%d) value 0x%02x (%d)\n", address, address, data, data);
    }
}
