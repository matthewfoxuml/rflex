...
//ROS 2 STARTS BELOW
//To uncomment out the code, delete any asteriks and slashes from the left margins, along with
//the ones at the absolute top and bottom
...
/*
        ...
        If we take the absolute difference of each pair, the pair which produces the min were closest to one another
        */
/*      v_X = std::abs(v_A - v_B); //take the absolute differences
        v_Y = std::abs(v_A - v_C);
        v_Z = std::abs(v_B - v_C);
        ...
        std::cout << std::setprecision(12) << "Dist: " << Distance << std::endl; //print distance left
        driver.driveForwardSafely(4); //move forward 4 meters with a tapered speed at the end
        ros::spinOnce(); //get new data
    } while (Distance > 6); //check whether we are within 6m of our target
